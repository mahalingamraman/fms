...
import * as Form5Actions from '../../../actions/oht/velocity_forms_actions/form5'
...
var usnType = {"Select":"Select","usn":"usn"}
var usnTypeArr = ["Select","usn"]
  ...
  render() {
    return (<div className="Another">
      ...
      fontFamily: "'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif"} }
        ...
        <TableHeaderColumn  dataSort={true} dataField='stop_time_type' editable={{ type: 'select', options: { values: usnTypeArr } }} dataFormat={enumFormatter} filterFormatted={true} formatExtraData={usnType}
          filter={{ type: 'SelectFilter', options: usnType }}>USN</TableHeaderColumn>
        
        <TableHeaderColumn filter={{ type: 'DateFilter' }} editable={{ type: 'datetime-local'}} dataSort={true} dataField='sol'>SOL</TableHeaderColumn>
        <TableHeaderColumn filter={{ type: 'DateFilter' }} editable={{ type: 'datetime-local'}} dataSort={true} dataField='fol'>FOL</TableHeaderColumn>
...
function mapStateToProps(state) {
  return {
    table: state.form5Reducer	
  }
}
function mapDispatchToProps(dispatch) {
  return {
    actions: bindActionCreators(Form5Actions, dispatch)
  }
}
